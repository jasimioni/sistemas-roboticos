...
class MoverFuzzy(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(self.loop_interval, self.fuzzy_set_position_callback)
    ...
    def _setup_fuzzy_system(self):
        ...
        # 
        distance['near'] = fuzz.trimf(distance.universe, [0, 0, 1.0])
        distance['medium'] = fuzz.trimf(distance.universe, [0, 1.0, 2.0])
        distance['far'] = fuzz.trimf(distance.universe, [1.0, 5, 5])
        ...
        linear_velocity['slow'] = fuzz.trimf(linear_velocity.universe, [0, 0, 0.2])
        linear_velocity['fast'] = fuzz.trimf(linear_velocity.universe, [0, 0.5, 0.5])
        
        angular_velocity['turn_right'] = fuzz.trimf(angular_velocity.universe, [-1.0, -1.0, 0])
        angular_velocity['straight'] = fuzz.trimf(angular_velocity.universe, [-0.2, 0, 0.2])
        angular_velocity['turn_left'] = fuzz.trimf(angular_velocity.universe, [0, 1.0, 1.0])
        
        # --- 3. Define the Fuzzy Rules ---
        rules = [
            ctrl.Rule(distance['far'] & angle_error['zero'], linear_velocity['fast']),
            ctrl.Rule(distance['medium'] & angle_error['zero'], linear_velocity['fast']),
    ...
    def fuzzy_set_position_callback(self):
        msg = self.last_pose
        ...
        self.fuzzy_sim.input['distance'] = distance_error
        self.fuzzy_sim.input['angle_error'] = angle_error
        
        # 2. Compute the result
        self.fuzzy_sim.compute()
        
        # 3. Get the crisp (defuzzified) output values
