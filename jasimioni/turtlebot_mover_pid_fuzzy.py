...
class MoverPID(Node):
    def __init__(self):
        ...
        self.orientation_pid = PIDController(kp=1.5, ki=0.2, kd=0.1)
        
        self.loop_interval = 0.1 # seconds
        self.circle_control = []
        
        self.fuzzy_sim = self._setup_fuzzy_system()
    ...
    def _setup_fuzzy_system(self):
        """Creates and configures the fuzzy control system."""
        # --- 1. Define Antecedents (Inputs) and Consequents (Outputs) ---
        # Universe of discourse: the range of possible values for each variable.
        distance = ctrl.Antecedent(np.arange(0, 5, 0.1), 'distance')       # 0 to 5 meters
        angle_error = ctrl.Antecedent(np.arange(-math.pi, math.pi, 0.1), 'angle_error') # -180 to 180 degrees
        
        linear_velocity = ctrl.Consequent(np.arange(0, 0.5, 0.01), 'linear_velocity') # 0 to 0.5 m/s
        angular_velocity = ctrl.Consequent(np.arange(-1.0, 1.0, 0.1), 'angular_velocity') # -1.0 to 1.0 rad/s

        # --- 2. Define Membership Functions (Linguistic Terms) ---
        # 
        distance['near'] = fuzz.trimf(distance.universe, [0, 0, 0.1])
        distance['medium'] = fuzz.trimf(distance.universe, [0, 0.2, 0.3])
        distance['far'] = fuzz.trimf(distance.universe, [0.2, 0.5, 0.5])
        
        angle_error['left'] = fuzz.trimf(angle_error.universe, [-math.pi, -math.pi, 0])
        angle_error['zero'] = fuzz.trimf(angle_error.universe, [-0.4, 0, 0.4])
        angle_error['right'] = fuzz.trimf(angle_error.universe, [0, math.pi, math.pi])

        linear_velocity['slow'] = fuzz.trimf(linear_velocity.universe, [0, 0, 0.0])
        linear_velocity['fast'] = fuzz.trimf(linear_velocity.universe, [0, 0.1, 0.1])
        
        angular_velocity['turn_right'] = fuzz.trimf(angular_velocity.universe, [-0.5, -0.5, 0])
        angular_velocity['straight'] = fuzz.trimf(angular_velocity.universe, [-0.1, 0, 0.1])
        angular_velocity['turn_left'] = fuzz.trimf(angular_velocity.universe, [0, 0.5, 0.5])
        
        # --- 3. Define the Fuzzy Rules ---
        rules = [
            ctrl.Rule(distance['far'], linear_velocity['fast']),
            ctrl.Rule(distance['medium'], linear_velocity['slow']),
            ctrl.Rule(distance['near'], linear_velocity['slow']),
            ctrl.Rule(angle_error['left'], angular_velocity['turn_right']),
            ctrl.Rule(angle_error['right'], angular_velocity['turn_left']),
            ctrl.Rule(angle_error['zero'], angular_velocity['straight'])
        ]

        # --- 4. Create the Control System Simulation ---
        robot_ctrl = ctrl.ControlSystem(rules)
        return ctrl.ControlSystemSimulation(robot_ctrl)
    ...
    def pid_set_position_callback(self):
        ...
        if distance_error < 0.1:
            self.get_logger().info(f'Reached the desired position. {current_position_x:.2f}, {current_position_y:.2f}')
        ...
        # Obstacle avoidance using LIDAR (Fuzzy Control for Angular and Linear Velocity)
        if self.last_scan is not None:
            ...
            if front_min < 0.3:
                # Where do I want to go if to close to an obstacle?
                if left_min > right_min:
                    self.get_logger().info('Moving to the Left')
                    # angular_velocity = 0.5
                    desired_angle = theta + math.pi / 4
                else:
                ...
                self.get_logger().info(f'Fuzzy Inputs -> Distance: {front_min:.2f}, Angle Error: {angle_error:.2f}')
                
                self.fuzzy_sim.input['distance'] = front_min
                self.fuzzy_sim.input['angle_error'] = angle_error
                ...
                try:
                    ...
                    self.get_logger().info('Fuzzy output error, keeping previous linear velocity')
                    linear_velocity = self.linear_velocity
                
                try:
                    angular_velocity = self.fuzzy_sim.output['angular_velocity']
                except:
                    self.get_logger().info('Fuzzy output error, keeping previous angular velocity')
                    angular_velocity = self.angular_velocity 
                
                self.get_logger().info(f'Fuzzy Outputs -> Linear Velocity: {linear_velocity:.2f}, Angular Velocity: {angular_velocity:.2f}')                
